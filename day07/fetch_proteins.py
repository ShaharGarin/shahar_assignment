...
def search_protein():
    if len(sys.argv) != 3 or not sys.argv[2].isnumeric():
        raise Exception("Please provide search term and numberof files to download")
    if float(sys.argv[2]) > file_limit:
        raise Exception("Too many files. NCBI limits download to 10,000")
...
def files_download(search_dic):
    for ide in search_dic["IdList"]:
        ...
        file = f"{sys.argv[1]} sequence {ide}"
...
def log_maker(search_dic):
    log_name = "protein search log.csv"
